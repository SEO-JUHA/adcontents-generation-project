...
def _guess_input_dir() -> Optional[str]:
    ...
    here = Path(__file__).resolve()
    guess = here.parents[1] / "ComfyUI" / "input"
    return str(guess)
...
def stage_to_input(local_path: str) -> str:
    ...
    poster_root = Path(__file__).resolve().parents[1]  # /.../poster
    try:
        rel_from_poster = src.relative_to(poster_root)  # e.g., data/logos/...
        # poster 하위 파일만 경로 보존 대상
        if rel_from_poster.parts and rel_from_poster.parts[0] == "data":
            dst = (input_dir_p / rel_from_poster).resolve()  # /.../input/data/...
            ...
            return str(rel_from_poster).replace("\\", "/")
    except Exception:
        pass
...
def post_prompt(prompt_json: Dict[str, Any]) -> str:
    ...
    payload = {"prompt": prompt_clean, "client_id": "adposter-cli"}
...
def wait_and_collect(prompt_id: str) -> List[str]:
    ...
    poll = float(cfg.get("poll_interval", 0.75))
    save_dir_hint = cfg.get("save_dir_hint", "ComfyUI")

    if not os.path.isabs(save_dir_hint):
        here = Path(__file__).resolve()
        base = str((here.parents[1] / save_dir_hint).resolve())
    else:
        base = save_dir_hint
