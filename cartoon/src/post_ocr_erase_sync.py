...
def ocr_boxes(img_path: str, slot_mask_L: Image.Image,
              engine_kind: str | None, engine_cls, min_conf=0.5, lang="en",
    ...
    if engine_kind == "paddle":
        ...
        for line in res:
            for det in line:
                ...
                if slot_mask_L.getpixel((cx,cy))>0:
                    if return_text:
                        boxes.append({"bbox":[x1,y1,x2,y2],"conf":float(conf),"text":str(txt)})
                    else:
        ...
        for i in range(n):
            ...
            if 0<=cx<W and 0<=cy<H and slot_mask_L.getpixel((cx,cy))>0:
                if return_text:
                    boxes.append({"bbox":[x,y,x+w,y+h], "conf":float(conf/100.0), "text":str(data["text"][i] or "")})
                else:
...
def erase_text_regions(
    ...
    *,
    ...
    slot_L = slot_L or Image.new("L",(W,H),255)

    kind, engine_cls = _try_imports(engine if engine!="none" else None)

    if keywords:
        dets = ocr_boxes(image_path, slot_L, kind, engine_cls, min_conf=min_conf, lang=lang, return_text=True)
        rects = []
        y_cut = int(H * (1.0 - max(0.0, min(1.0, bottom_ratio))))  # 예: 0.5 -> 하단 50%
        kws = [k.lower() for k in keywords]
        for d in dets:
            txt = (d.get("text") or "").lower()
            x1,y1,x2,y2 = d["bbox"]
            cy = (y1+y2)//2
            if cy >= y_cut and any(k in txt for k in kws):
                rects.append([x1,y1,x2,y2,float(d.get("conf",1.0))])
    else:
        rects = ocr_boxes(image_path, slot_L, kind, engine_cls, min_conf=min_conf, lang=lang)
    ...
    if mask_mode == "boxes":
        ...
        final_L = union_mask(slot_L, boxes_L)
...
router = APIRouter(tags=["ocr-erase"])

class EraseTextRequest(BaseModel):
    image_path: str
    out_dir: str
    slots_mask_path: Optional[str] = None
    engine: str = "paddle"
    lang: str = "korean"
    min_conf: float = 0.5
    boxes_pad: int = 14
...
@router.post("/erase", response_model=EraseTextResponse)
async def api_erase_text(req: EraseTextRequest):
    try:
        out = erase_text_regions(
            image_path=req.image_path, slots_mask_path=req.slots_mask_path, out_dir=req.out_dir,
            engine=req.engine, lang=req.lang, min_conf=req.min_conf, boxes_pad=req.boxes_pad,
            mask_mode=req.mask_mode, method=req.method, radius=req.radius, tone=req.tone,
            alpha=req.alpha, write_debug=req.write_debug, keywords=req.keywords, bottom_ratio=req.bottom_ratio,
        )
        return EraseTextResponse(out_path=out)
    except SystemExit as e:
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"erase_text_regions failed: {e}")
